...
namespace TOH.Core
{
	public class Parameters 
	{
		...
		public IEnumerable<KeyValuePair<string, string>> ForEachString()
		{
			foreach (KeyValuePair<string, string> stringPair in this.stringData)
			{
		...
		public void PutExtra(string paramName, short value) {
			if(this.shortData.ContainsKey(paramName)) {
				this.shortData[paramName] = value;
			} else {
